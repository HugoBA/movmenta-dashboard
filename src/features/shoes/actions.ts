...
import { deleteShoe } from "@/lib/xano/shoes";
...
// makes — space deletes out so a big selection doesn't immediately 429.
...
export async function deleteShoes(
  ...
  for (let i = 0; i < ids.length; i++) {
    try {
      await deleteShoe(session.token, ids[i]);
      deletedCount++;
    } catch (err) {
      if (err instanceof XanoApiError && err.status === 404) {
        return {
          deletedCount,
          error: "Delete isn't available yet — add DELETE /admin/shoe/{id} in Xano first.",
        };
      }
