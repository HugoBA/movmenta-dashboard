...
interface Column {
  ...
  label: string;
  align?: "right";
  render: (row: ShoeBrandRecord) => React.ReactNode;
  sortValue: (row: ShoeBrandRecord) => string | number;
  csv: (row: ShoeBrandRecord) => string | number;
}

type SortDirection = "asc" | "desc";

export interface ShoeBrandsTableHandle {
  getExportRows: () => CsvRow[];
  exportHeaders: string[];
}

export const ShoeBrandsTable = forwardRef<
  ShoeBrandsTableHandle,
  {
    data: ShoeBrandRecord[];
    selectedIds: Set<number>;
    onSelectedIdsChange: (ids: Set<number>) => void;
  }
>(function ShoeBrandsTable({ data, selectedIds, onSelectedIdsChange }, ref) {
  ...
  const columns: Column[] = [
    {
      key: "brand_name",
      label: "Brand",
      render: (row) => <span className="font-semibold">{row.brand_name}</span>,
      sortValue: (row) => row.brand_name,
      csv: (row) => row.brand_name,
    },
    {
      key: "created_at",
      label: "Created",
